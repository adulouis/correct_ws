...
public:
    ...
    : Node("simple_action_server", options)
  {
    ...
    action_server_ = rclcpp_action::create_server<correct_msgs::action::Fibonacci>(
...
private:
  rclcpp_action::Server<correct_msgs::action::Fibonacci>::SharedPtr action_server_;

  rclcpp_action::GoalResponse goalCallback(
      const rclcpp_action::GoalUUID& uuid,
      std::shared_ptr<const correct_msgs::action::Fibonacci::Goal> goal)
  {
  ...
  void execute(const std::shared_ptr<rclcpp_action::ServerGoalHandle<correct_msgs::action::Fibonacci>> goal_handle)
  {
    ...
    auto feedback = std::make_shared<correct_msgs::action::Fibonacci::Feedback>();
    auto& sequence = feedback->partial_sequence;
    sequence.push_back(0);
    sequence.push_back(1);
    auto result = std::make_shared<correct_msgs::action::Fibonacci::Result>();
