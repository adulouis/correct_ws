...
class SimpleLifecycleNode : public rclcpp_lifecycle::LifecycleNode
{
    ...
    rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn on_configure(const rclcpp_lifecycle::State &){
        sub = create_subscription<std_msgs::msg::String>("my_lifecycle_string",10,std::bind(&SimpleLifecycleNode::msgCallback, this, _1));
        RCLCPP_INFO_STREAM(get_logger(), "On configure state up!");
        return rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn::SUCCESS;
    }

    rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn on_shutdown(const rclcpp_lifecycle::State &){
        sub.reset();
        RCLCPP_INFO_STREAM(get_logger(), "On shutdown state up!");
        return rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn::SUCCESS;
    }

    rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn on_cleanup(const rclcpp_lifecycle::State &){
        sub.reset();
        RCLCPP_INFO_STREAM(get_logger(), "On cleanup state up!");
        return rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn::SUCCESS;
    }

    rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn on_activate(const rclcpp_lifecycle::State &state){
        LifecycleNode::on_activate(state);
        RCLCPP_INFO_STREAM(get_logger(), "On activate state up!");
