...
#include "correct_planning/djikstra_planning.hpp"
#include "rmw/qos_profiles.h"


namespace correct_planning
{
DjikstraPlanner::DjikstraPlanner() : Node("dijkstra_node")
{
...
nav_msgs::msg::Path DjikstraPlanner::plan(const geometry_msgs::msg::Pose & start, const geometry_msgs::msg::Pose & goal)
{
    ...
    while (!pending_nodes.empty() && rclcpp::ok()) {
        ...
        visited_map.data.at(poseToCell(active_node)) = 10;  // Blue
...
bool DjikstraPlanner::poseOnMap(const GraphNode & node)
{
    return node.x < static_cast<int>(map->info.width) && node.x >= 0 &&
        node.y < static_cast<int>(map->info.height) && node.y >= 0;
}

GraphNode DjikstraPlanner::worldToGrid(const geometry_msgs::msg::Pose & pose)
{
...
geometry_msgs::msg::Pose DjikstraPlanner::gridToWorld(const GraphNode & node)
{
...
unsigned int DjikstraPlanner::poseToCell(const GraphNode & node)
{
...
int main(int argc, char **argv)
{
    ...
    auto node = std::make_shared<correct_planning::DjikstraPlanner>();
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}